...
type Tup = [Piece, number];

export class VirtualGameBoard {
    // private board: Piece[];
    static virtualGameBoardsArray: any[] = [];
    static counter = 0;
    static deadStack: Piece[] = [];
    ...
    static updateIndexesOfPieces(piece: Piece, newIndex: number) {
    ...
    static isFieldEmpty(index: number) {
        let lastAr = this.virtualGameBoardsArray[this.virtualGameBoardsArray.length - 1];
    ...
    static findPieceIndexInSpecifiedBoard(index: number, ar: any) {
        let lastAr = ar;
    ...
    static findPieceInVirtualBoard(index: number) {
        let lastAr = this.virtualGameBoardsArray[this.virtualGameBoardsArray.length - 1];
        // console.log('lastAr:', lastAr);
    ...
    static getArOfPiecesOfSpecifiedType(pieceType: CustomTypes.PieceType , color :CustomTypes.Color) {
        let lastAr = this.virtualGameBoardsArray[this.virtualGameBoardsArray.length - 1];
        const piecesAr = [];
    ...
    static deletePieceFromVirtualBoard(index: number) {
        let lastAr = this.virtualGameBoardsArray[this.virtualGameBoardsArray.length - 1];
    ...
    static addToDeadStack(pieceIndex: number) {
        let lastAr = this.virtualGameBoardsArray[this.virtualGameBoardsArray.length - 1];
    ...
    */
    ...
    static mapVirtualBoardToGameBoard(board: any) {
        ...
        // console.log(board);
        for(let i = 0; i < board.length; i++) {
            (board[i][0] as Piece).appendPieceToBoard(board[i][1] as number);
            (board[i][0] as Piece).appendImgToBoard(board[i][1] as number);
            // console.log(board[i][0]);
            // console.log(board[i][1]);
